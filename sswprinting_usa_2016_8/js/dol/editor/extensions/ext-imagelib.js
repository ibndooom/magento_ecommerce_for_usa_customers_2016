...
// 				});
	...
	function toggleMulti(show) {
		...
		if(!preview) {
			...
			submit = $j('<button disabled>Import selected</button>').appendTo('#imgbrowse').click(function() {
	...
	function showBrowser() {
		...
		if(!browser.length) {
			...
			var cancel = $j('<button>' + uiStrings.common.cancel + '</button>').appendTo(browser).click(function() {
				$j('#imgbrowse_holder').hide();
			}).css({
				position: 'absolute',
				top: 5,
			...
			var back = $j('<button hidden>' + uiStrings.imagelib.show_list + '</button>').appendTo(leftBlock).click(function() {
			...
			$j.each(img_libs, function(i, opts) {
				$j('<li>').appendTo(lib_opts).text(opts.name).click(function() {
