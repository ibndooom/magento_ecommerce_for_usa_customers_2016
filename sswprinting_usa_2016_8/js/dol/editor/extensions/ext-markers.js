...
svgEditor.addExtension("Markers", function(S) {
	...
	selElems;
	console.log("svgEditor.curConfig.imgPath");
	...
	$j.each(['leftarrow','rightarrow','box','star','mcircle','triangle'],function(i,v) {
	...
	function showPanel(on) {
		...
		if(on) {
			...
			$j.each(mtypes, function(i, pos) {
				var m=getLinked(el,"marker-"+pos);
				var txtbox = $j('#'+pos+'_marker');
				if (!m) {
					val='\\nomarker';
					ci=val;
		...
		if(selElems[0]!=null && selElems[0]!=false ){
			if(selElems[0].getAttribute('lock')!='true'){
				$j('.marker_panel').toggle(on);
			}else{
				$j('.marker_panel').toggle(false);
			}
		}
	}	
	...
	function convertline(elem) {
		...
		$j.each(mtypes, function(i, pos) { // get any existing marker definitions
			var nam = 'marker-'+pos;
	...
	function updateReferences(el) {
		$j.each(mtypes, function (i,pos) {
			...
			if (url) {
				...
				if (el.id != linkid) {
					var val = $j('#'+pos+'_marker').attr('value');
					addMarker(id, val);
					svgCanvas.changeSelectedAttribute(marker_name, "url(#" + id + ")");
					if (el.tagName == "line" && pos=='mid') el=convertline(el);
					S.call("changed", selElems);
	...
	function triggerTextEntry(pos,val) {
		$j('#'+pos+'_marker').val(val);
		$j('#'+pos+'_marker').change();
		var txtbox = $j('#'+pos+'_marker');
	...
	function setIcon(pos,id) {
		...
		svgEditor.setIcon('#cur_' + pos +'_marker_list', $j(ci).children());
		$j(ci).addClass('current').siblings().removeClass('current');
	}
	...
	function showTextPrompt(pos) {
		var def = $j('#'+pos+'_marker').val();
		if (def.substr(0,1)=='\\') def='';
		$j.prompt('Enter text for ' + pos + ' marker', def , function(txt) { if (txt) triggerTextEntry(pos,txt); });
	}
...
*/
		$j.each(mtypes,function(k,pos) {
			var listname = pos + "_marker_list";
			var def = true;
		$j.each(marker_types,function(id,v) {
	...
	return {
		...
		callback: function() {
			$j('#marker_panel').addClass('toolset').hide();
	...
	selectedChanged: function(opts) {
		...
		while(i--) {
			var elem = selElems[i];
			if(elem && $j.inArray(elem.tagName, marker_elems) != -1) {
