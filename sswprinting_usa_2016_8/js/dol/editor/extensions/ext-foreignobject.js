...
svgEditor.addExtension("foreignObject", function(S) {
		...
		function showPanel(on) {
			var fc_rules = $j('#fc_rules');
			if(!fc_rules.length) {
				fc_rules = $j('<style id="fc_rules"><\/style>').appendTo('head');
			} 
			fc_rules.text(!on?"":" #tool_topath { display: none !important; }");
			$j('#foreignObject_panel').toggle(on);
		}

		function toggleSourceButtons(on) {
			$j('#tool_source_save, #tool_source_cancel').toggle(!on);
			$j('#foreign_save, #foreign_cancel').toggle(on);
		}
		...
		return {
			...
			callback: function() {
				...
				setTimeout(function() {				
					...
					var save = $j('#tool_source_save').clone()
						.hide().attr('id', 'foreign_save').unbind()
						.appendTo("#tool_source_back").click(function() {
							
							if (!editingforeign) return;

							if (!setForeignString($j('#svg_source_textarea').val())) {
								$j.confirm("Errors found. Revert to original?", function(ok) {
									if(!ok) return false;
									endChanges();
								});
							} else {
					...
					var cancel = $j('#tool_source_cancel').clone()
			...
			mouseUp: function(opts) {
				...
				if(svgCanvas.getMode() == "foreign" && started) {
					var attrs = $j(newFO).attr(["width", "height"]);
				...
				while(i--) {
					...
					if(elem && elem.tagName == "foreignObject") {
						if(opts.selectedElement && !opts.multiselected) {
							$j('#foreign_font_size').val(elem.getAttribute("font-size"));
							$j('#foreign_width').val(elem.getAttribute("width"));
							$j('#foreign_height').val(elem.getAttribute("height"));
