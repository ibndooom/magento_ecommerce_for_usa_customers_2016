...
// 		var give_offset = $j(line).data(side+'_off');
	...
	function showPanel(on) {
		var conn_rules = $j('#connector_rules');
		if(!conn_rules.length) {
			conn_rules = $j('<style id="connector_rules"><\/style>').appendTo('head');
		} 
		conn_rules.text(!on?"":"#tool_clone, #tool_topath, #tool_angle, #xy_panel { display: none !important; }");
		$j('#connector_panel').toggle(on);
	}
...
// 						var sw = line.getAttribute('stroke-width') * 5;
			...
			var pt = getBBintersect(src_x, src_y, bb, getOffset(pre, line)); // $j(line).data(pre+'_off')?sw:0
	...
	function findConnectors(elems) {
		...
		var connectors = $j(svgcontent).find(conn_sel);
		connections = [];
		...
		connectors.each(function() {
			...
			for(var i=0; i<2; i++) {
				...
				$j(c_elem).parents().each(function() {
					if($j.inArray(this, elems) !== -1) {
				...
				if($j.inArray(c_elem, elems) !== -1 || add_this) {
	...
	(function() {
		...
		svgCanvas.groupSelectedElements = function() {
			svgCanvas.removeFromSelection($j(conn_sel).toArray());
	...
	function init() {
		...
		$j(svgcontent).find('*').each(function() { 
			...
			if(conn) {
				...
				$j(this).data('c_start',conn_data[0])
...
// 		$j(svgroot).parent().mousemove(function(e) {
// // 			if(started 
	...
	return {
		...
		mouseDown: function(opts) {
			...
			if(mode == "connector") {
				...
				var parents = $j(mouse_target).parents();
				
				if($j.inArray(svgcontent, parents) != -1) {
					// Connectable element
					
					// If child of foreignObject, use parent
					var fo = $j(mouse_target).closest("foreignObject");
		...
		mouseUp: function(opts) {
			...
			if(svgCanvas.getMode() == "connector") {
				...
				if(mouse_target == start_elem) {
				...
				} else if($j.inArray(svgcontent, parents) === -1) {
					...
					var dupe = $j(svgcontent).find(conn_sel).filter(function() {
						var conn = this.getAttributeNS(se_ns, "connector");
		...
		selectedChanged: function(opts) {
			...
			if(!$j(svgcontent).find(conn_sel).length) return;
		...
		toolButtonStateUpdate: function(opts) {
			if(opts.nostroke) {
				if ($j('#mode_connect').hasClass('tool_button_current')) {
					clickSelect();
				}
			}
			$j('#mode_connect')
