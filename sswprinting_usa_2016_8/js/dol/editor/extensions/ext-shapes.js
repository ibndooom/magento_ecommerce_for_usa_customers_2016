...
svgEditor.addExtension("shapes", function() {
	...
	function loadLibrary(cat_id) {
		...
		if(!lib) {
			var bar = $j('#updating_box #updating_box_container .desc img').attr('src');
			$j('#shape_buttons').html('<div class="rendering">Loading Buttons....... <img src='+bar+'></div>');
		...
		if(shapes.data){
			...
			$j.each(obj, function(i, elem) {
				...
				if(type == "image" || type == "svg"){
				   ...
				   icon_btn = icon_btn.wrapInner('<img class="tool_button" src="'+ thumbnail +'" orig_url="'+ path_d +'" title="'+ icon_name +'" alt="'+icon_name+'">');
				}
...
//			}').appendTo('head');
			...
			$j('#shape_buttons').mouseup(function(evt) {
				...
				if(current_d == null || current_d == ""){
					$j.each(cur_lib.data, function(i, elem) {
						var elem_id = elem.clipartName;
						var elem_type = elem.clipartType;
						
						if(elem_type == 'path' && elem_id == cur_shape_id){
							current_d = elem.clipartImage;
						}else if(elem_type == "svg" && elem_id == cur_shape_id){
							$j('#updating_box').show();
							$j('#updating_box .desc').show();
						    $j('#updating_box .desc-rendering').hide();
							$j.ajax({
								'url': elem.clipartImage,
								 type: 'GET',
								 async: false,
								 dataType: 'text',
								 success: function(data) {
						...
						}else if(elem_type == "image" && elem_id == cur_shape_id){
							canv.setMode('select');
							canv.addGalleryImage(elem.clipartImage, true);
						}
					});
				}
				...
				var x = start_x = svgcont_x = ($j('#svgcontent').attr('width')/zoom)/2;
				var y = start_y = svgcont_y = ($j('#svgcontent').attr('height')/zoom)/2;
				
				var pt_size = 150/zoom;
				var x1 =  x + pt_size;
				var y1 =  y + pt_size;
				...
				if(btn != 'raphael_1' && btn != 'raphael_2'){
					translateOrigin.setTranslate(-(svgcont_x+tx), -(svgcont_y+tx));
				}
				...
				translateBack.setTranslate(svgcont_x+tx, svgcont_y+tx);
