...
svgEditor.addExtension("server_opensave", {
	callback: function() {
		...
		function rebuildInput(form) {
			form.empty();
			var inp = $j('<input type="file" name="svg_file">').appendTo(form);
			...
			function submit() {
				...
				rebuildInput(form);
				$j.process_cancel("Uploading...", function() {
					cancelled = true;
					$j('#dialog_box').hide();
				});
			}
		...
		$j("#tool_open").show().prepend(open_svg_form);
		$j("#tool_import").show().prepend(import_svg_form);
		$j("#tool_image").prepend(import_img_form);
	}
});
