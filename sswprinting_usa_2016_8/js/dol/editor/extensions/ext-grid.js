...
svgEditor.addExtension("view_grid", function(s) {
    ...
    var canvBG = $j('#canvasBackground');
   // canvBG.append(canvasgrid);
    $j('#svgroot').append(canvasgrid);
        // grid-pattern
...
// 					var line_num = (i % 2)?12:10;
    ...
    return {
        ...
        buttons: [{
            ...
            events: {
                'click': function() {
                    var gr = !$j('#view_grid').hasClass('push_button_pressed');
                    if (gr) {
                        svgEditor.curConfig.showGrid = showGrid = true;
                        $j('#view_grid').addClass('push_button_pressed').removeClass('tool_button');
                        $j('#canvasGrid').attr('display', 'normal');
                        updateGrid(svgCanvas.getZoom());
                    }
                    else {
                        svgEditor.curConfig.showGrid = showGrid = false;
                        $j('#view_grid').removeClass('push_button_pressed').addClass('tool_button');
                        $j('#canvasGrid').attr('display', 'none');
                    }
                }
            }
}]
