...
svgEditor.addExtension("shapes", function() {
	...
	var lastBBox = {};
	var clipart_url = '/clipart/index/all';
...
//		raphael_2: 'raphaeljs.com set 2'
	...
	var clipart_array;	
	if(svgEditor.curConfig.crossDomain=="" || svgEditor.curConfig.crossDomain==undefined){
		$j.ajax({
			  url: clipart_url,
			  dataType: 'json',
			  async: false,
			  success: function(data){
				var obj = data.clipart;
				var new_array = {};
		
				$j.each(obj, function(i, elem) {
					var btn = elem.clipartCategory;
						
					for (var key in categories) {
				        if(key === btn) {
				            return true;
				        }else{
				        	var cat = btn.toLowerCase().split(' ').join('');
				        	new_array[cat] = btn.substr(0, 1).toUpperCase() + btn.substr(1);
				        }
				    }
				});
	
				clipart_array = new_array;
				if(canv.loadDefaultCliparts()){
					categories = merge_options(new_array,categories);
				}else{
		...
		$j.ajax({
			  url: "http://" + svgEditor.curConfig.crossDomain + clipart_url+"?callback=?",			  
			  async: false,
	...
	function checkOldCategory(cat_id){
		if(cat_id === 'basic' || cat_id === 'object' || cat_id === 'symbol' || cat_id === 'arrow' || cat_id === 'flowchart' ||
				 cat_id === 'animal' || cat_id === 'game' || cat_id === 'dialog_balloon' || cat_id === 'electronics' || cat_id === 'math' || 
				 cat_id === 'music' || cat_id === 'misc' || cat_id === 'raphael_1' || cat_id === 'raphael_2'){
				return true;
		}else{
			return false;
		}
	...
	function loadIcons() {
		$j('#shape_buttons').empty();
		
		// Show lib ones
		$j('#shape_buttons').append(cur_lib.buttons);
	}
	
	function loadLibrary(cat_id) {
		var btn_url = '/js/all/editor/extensions/shapelib/' + cat_id + '.json';
		if(clipart_array != null){
			for (var key in clipart_array) {
		        if(key === cat_id && !checkOldCategory(cat_id)) {
		        	btn_url = clipart_url;
		        }else if(checkOldCategory(cat_id)){
		        	btn_url = '/js/all/editor/extensions/shapelib/' + cat_id + '.json';
		        }
		    }
		}else{
			btn_url = '/js/all/editor/extensions/shapelib/' + cat_id + '.json';
		...
		if(!lib) {
			var bar = '/js/all/editor/icons/ajax-loader.gif';
			$j('#shape_buttons').html('<div class="rendering"><img src='+bar+'></div>');
			
			$j.getJSON(btn_url, function(result, textStatus) {
				if(result.clipart){
		...
		if(svgedit.browser.isIE()){
			if(cat_id == 'basic') makeButtons(cat_id, lib); // basic buttons are not loading in IE
		}
		loadIcons();
	}
	
	function makeButtons(cat, shapes) {
		$j('#shape_buttons').attr('attr', cat);
		...
		var svg_elem = $j(document.importNode(shape_icon.documentElement,true));
		...
		if(shapes.data){
			for(var id in data) {
				...
				if(id.substr(0,5) == 'raph_'){ // only for  buttons  of raphael categories
					var size = 32;
					var off = size * .05;
					var vb = [-off, -off, size + off*2, size + off*2].join(' ');
					var stroke = fill ? 0: (size/30);
					var svg = (icon_btn[0].firstChild).firstChild;
					svg.setAttribute('viewBox', vb);
					svg.firstChild.setAttribute('stroke-width', stroke);
				}
			...
			var obj = shapes.clipart;
			$j.each(obj, function(i, elem) {
				var clip_no = elem.clipartId;
				var id = elem.clipartName;
				var path_d = elem.clipartImage;
				var thumbnail = elem.clipartThumb;
				var icon = svg_elem.clone();
				var type = elem.clipartType;
				var category = elem.clipartCategory;
				var icon_name = elem.clipartName;
				id = mode_id + '_' + id;
				//icon.find('path').attr('d', path_d);
				
				var icon_btn = icon.wrap('<div class="tool_button" attr_type="'+ type +'">').parent().attr({
					id: id
				});
				
				if(type == "image" || type == "svg"){
				   icon_btn.find('svg').remove();
				   icon_btn = icon_btn.wrapInner('<img class="tool_button" src="'+ thumbnail +'" orig_url="'+ path_d +'" title="'+ icon_name +'" alt="'+icon_name+'" clipart_id="'+clip_no+'" type="'+type+'">');
				}
				
				// Store for later use
				var current_cat = category.toLowerCase().split(' ').join('');
				if(current_cat == cat){
					cur_lib.buttons.push(icon_btn[0]);
				}else{
					if(checkOldCategory(current_cat)){
						var old_lib = library[current_cat];
						if(old_lib != undefined){
							for(i=0; i<old_lib.buttons.length; i++){
								if(old_lib.buttons[i].id === icon_btn[0].id){
									return;
								}
							}
							old_lib.buttons.push(icon_btn[0]);
							old_lib.data[id] = elem;
						}
					}
				}
			});
...
//			}').appendTo('head');
			...
			var btn_div = $j('<div id="shape_buttons">');
			$j('#tools_shapelib > *').wrapAll(btn_div);
			
			var shower = $j('#tools_shapelib_show');
			
//			if(clipart_array != null){
//				loadLibrary('object');
//				loadLibrary('symbol');
//				loadLibrary('arrow');
//				loadLibrary('flowchart');
//				loadLibrary('animal');
//				loadLibrary('game');
//				loadLibrary('dialog_balloon');
//				loadLibrary('electronics');
//				loadLibrary('math');
//				loadLibrary('music');
//				loadLibrary('misc');
//				loadLibrary('raphael_1');
//				loadLibrary('raphael_2');
//			}
			// For IE only, not loading buttons for 2nd time click on tabs 
			if(svgedit.browser.isIE()){
				for (var key in categories) {
					loadLibrary(key);
				}
			...
			loadLibrary('basic');
			for(var key in categories) break;
			loadLibrary(key);
			
			setTimeout(function(){
				var cur_attr = $j('#shape_buttons').attr('attr');
				var cur_btn = $j('#shape_cats .current').attr('data-cat');
				if(cur_attr != cur_btn){
					loadLibrary(key);
				}
				
				// increase popup height dynamically
				var cat_no = $j("#shape_cats div").length;
				if(cat_no < 15) cat_no = 15;
				var t = 32;
				var pop_h = cat_no*(t+1);
				if(pop_h > 480) pop_h = 500;
				$j("#shapes_lib").parent().css('height', (pop_h+160)+'px');
				$j("#shapes_lib").css('height', (pop_h+90)+'px');
				
				$j('#shapes_lib').parent().position({
					my: "center center",
					at: "center center",
					of: window
					});
			...
			$j('#shape_buttons').mouseup(function(evt) {
				var btn = $j(evt.target).closest('div.tool_button');
				...
				var cust_btn = btn.children()[0];
				
				if(current_d == null || current_d == ""){
						var elem_id = cust_btn.getAttribute('clipartName');
						var elem_type = cust_btn.getAttribute('type');
						var clip_url = cust_btn.getAttribute('orig_url');
						if(elem_type == 'path'){
							current_d = elem.clipartImage;
						}else if(elem_type == "svg"){							
							$j('#updating_box').show();
							...
							$j.ajax({
								'url': clip_url,
								 ...
								 dataType: 'text',
								 success: function(data) {
									 canv.importSvgString(data,true);
									 $j('#updating_box .desc').hide();
								     $j('#updating_box .desc-rendering').show();
									 canv.ungroupSelectedElement();
									 canv.ungroupSelectedElement();
									 canv.groupSelectedElements(null, true);
									 
									 var zoom = svgCanvas.getZoom();
									 if(svgEditor.curConfig.defaultImage_x=="center"){
										 canv.alignSelectedElements("m", "page");
									 }else{										 
										 canv.moveSelectedElements(svgEditor.curConfig.defaultImage_x*zoom,0)
									 }
									 if(svgEditor.curConfig.defaultImage_y=="center"){
										 canv.alignSelectedElements("c", "page");										
									 }else{
										 canv.moveSelectedElements(0,svgEditor.curConfig.defaultImage_y*zoom)
									 }
									 $j('#updating_box').hide();
									 $j('.clipart_colors .fill_box').children().remove();
									 $j('.clipart_colors .border_box').children().remove();
									 canv.getClipartColors();
									 if(svgEditor.curConfig.defaultImage_y=="center" && svgEditor.curConfig.defaultImage_x=="center"){
										 canv.centerCliparts();
									 }
									 return;
								}
							...
							canv.setMode('select');
							var clipart_img = new Image();
							clipart_img.src=clip_url;
							$j('#updating_box').show();
							$j('#updating_box .desc').show();
							$j(clipart_img).load(function(){
								origX = clipart_img.width;
								origY = clipart_img.height;
								canv.addClipartImage(clip_url,origX,origY);
							});
				...
				//$j('.tools_flyout').fadeOut();
				//$j('.ui-dialog-titlebar-close').trigger('click');
				
				//=== Start code for drop the shape directly center of the canvas =====
				var zoom = svgCanvas.getZoom();
				var x = start_x = svgcont_x = $j('#svgcontent').attr('x');
				var y = start_y = svgcont_y = $j('#svgcontent').attr('y');
				
				var x1 =  x + ($j('#svgcontent').attr('width')/3)/zoom;
				var y1 =  y + ($j('#svgcontent').attr('height')/3)/zoom;
				
				//========== mouseDown ===================
				//var mode = canv.getMode();
				//if(mode !== mode_id) return;
				canv.setMode('shapelib');
				...
				var dx = (x1-start_x), dy = (y1-start_y);
	
				var newbox = {
					'x': Math.min(start_x,x1),
					'y': Math.min(start_y,y1),
					'width': Math.abs(x1-start_x),
					'height': Math.abs(y1-start_y)
				};
				...
				if(y1 < start_y) ty = lastBBox.height;
...
//				var btn = $j('#shape_cats').find('.current').attr("data-cat");
//				if(btn != 'raphael_1' && btn != 'raphael_2'){
				translateOrigin.setTranslate(-((svgEditor.curConfig.defaultImage_x=="center")?svgcont_x:parseInt(svgEditor.curConfig.defaultImage_x)+parseInt(tx)), -((svgEditor.curConfig.defaultImage_y=="center")?svgcont_y:parseInt(svgEditor.curConfig.defaultImage_y)+parseInt(tx)));
//				}
				...
				translateBack.setTranslate((svgEditor.curConfig.defaultImage_x=="center")?svgcont_x:parseInt(svgEditor.curConfig.defaultImage_x)+tx, (svgEditor.curConfig.defaultImage_y=="center")?svgcont_y:parseInt(svgEditor.curConfig.defaultImage_y)+tx);
				var N = tlist.numberOfItems;
				tlist.appendItem(translateBack);
				tlist.appendItem(scale);
				tlist.appendItem(translateOrigin);
				cur_shape.setAttribute('attr_c', 'clipart_img');
				canv.recalculateDimensions(cur_shape);
				lastBBox = cur_shape.getBBox();
				cur_shape.setAttribute('attr_aspt', 'aspect');
				canv.setMode('select');
				canv.selectOnly([cur_shape], true);
				canv.alignSelectedElements("m", "page", true);
				canv.alignSelectedElements("c", "page", true);
				
			});
			//=================== end ===========
			
			var shape_cats = $j('<div id="shape_cats">');
			
			var cat_str = '';
			
			$j.each(categories, function(id, label) {
				cat_str += '<div data-cat=' + id + '>' + label + '<span class="arrow"></span>'+'</div>';
			});
			...
			$j('#tools_shapelib').append(shape_cats);

			shower.mouseup(function() {
				canv.setMode(current_d ? mode_id : 'select');
			});
			
			$j('#tool_shapelib').remove();
			
		  /*var h = $j('#tools_shapelib').height();
			$j('#tools_shapelib').css({
				'margin-top': -(h/2 - 15),
				'margin-left': 3
			});*/
		}/*,
