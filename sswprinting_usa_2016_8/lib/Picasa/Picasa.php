...
require_once 'lib/Picasa/Picasa/Image.php';
require_once 'lib/Picasa/Picasa/ImageCollection.php';
require_once 'lib/Picasa/Picasa/Album.php';
require_once 'lib/Picasa/Picasa/Account.php';
require_once 'lib/Picasa/Picasa/Comment.php';
require_once 'lib/Picasa/Picasa/Tag.php';
require_once 'lib/Picasa/Picasa/Author.php';
require_once 'lib/Picasa/Picasa/Exception.php';
require_once 'lib/Picasa/Picasa/Exception/CaptchaRequiredException.php';
require_once 'lib/Picasa/Picasa/Exception/UnauthorizedException.php';
require_once 'lib/Picasa/Picasa/Exception/BadRequestException.php';
require_once 'lib/Picasa/Picasa/Exception/FailedAuthorizationException.php';
require_once 'lib/Picasa/Picasa/Exception/FileNotFoundException.php';
require_once 'lib/Picasa/Picasa/Exception/InternalServerErrorException.php';
require_once 'lib/Picasa/Picasa/Exception/ConflictException.php';
require_once 'lib/Picasa/Picasa/Exception/InvalidUsernameOrPasswordException.php';
...
class Picasa {
	...
	public function getAlbumsByUsername($username, $maxResults=null, $startIndex=null, $visibility="public", $thumbsize=null, $imgmax=null) {
		...
		$account = null;
		try {
			$account = new Picasa_Account($query, null, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $account;
	 ...
	 * @link http://code.google.com/apis/picasaweb/reference.html#Parameters
	 */
	public function getImages($username=null, $maxResults=null, $startIndex=null, $keywords=null, $tags=null, $visibility="public", $thumbsize=null, $imgmax=null) {
		...
		$query .= '?kind=photo'.Picasa::buildQueryParams($maxResults, $startIndex, $keywords, $tags, $visibility, $thumbsize, $imgmax);
		$images = null;
		try {
			$images = new Picasa_ImageCollection($query, null, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $images;
	 ...
	 * @param string $tags     Space-delimited list of tags to search for.  Only images with all tags in the list are 
	 *                         included in the search results.  Optional, default is empty string.
	 * @param string $visibility Restrict the search to images with the access rights specified here.  Options are "public", 
	 *                           "private", and "all".  Authorization is required for "private" and "all" options.  Optional, 
	 *                           default is "public".
	 * @param int $thumbsize   Comma-delimited list of thumbnail sizes to fetch.  Options are 32, 48, 64, 72, 144, 160, 200, 
	 ...
	 * @link http://code.google.com/apis/picasaweb/reference.html#Parameters
	 */
	public function getAlbumById($username, $albumid, $maxResults=null, $startIndex=null, $keywords=null, $tags=null, $thumbsize=null, $imgmax='1600') {
	...
	public function getImageById($username, $albumid, $imageid, $thumbsize=null, $imgmax=null) {
		...
		if ($thumbsize !== null) {
	    	...
	    	try {
			$image = new Picasa_Image(Picasa::$BASE_QUERY_URL.'/user/'.$username.'/albumid/'.$albumid.'/photoid/'.$imageid.$params, null, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $image;
	...
	protected function getImageByIdAsEntry($username, $albumid, $imageid) {
	    	$image = null;
	    	try {
			$image = new Picasa_Image(Picasa::$BASE_ENTRY_QUERY_URL.'/user/'.$username.'/albumid/'.$albumid.'/photoid/'.$imageid, null, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $image;
	...
	public function getTagsByUsername($username, $albumid=null, $maxResults=null, $startIndex=null, $visibility="public") {
		...
		$tags = null;
	    	try {	    
			$tags = Picasa_Tag::getTagArray($query, null, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $tags;
	...
	public function getCommentById($username, $albumid, $imageid, $commentid) {
		...
		try {
			...
			$comment = new Picasa_Comment(null, Picasa::$BASE_ENTRY_QUERY_URL.'/user/'.$username.'/albumid/'.$albumid.'/photoid/'.$imageid.'/commentid/'.$commentid, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $comment;
	...
	public function getCommentsByUsername($username, $albumid=null, $maxResults=null, $startIndex=null, $visibility="public") {
		...
		$comments = null;
		try {
			$comments = Picasa_Comment::getCommentArray($query, null, $this->contextArray);
		} catch (Picasa_Exception $e) {	
			throw $e;
		}
		return $comments;
...
--END_OF_PART--";
	...
	public static function getExceptionFromInvalidPost($buf, $message=null) {
		if (preg_match("/401 UNAUTHORIZED/i", $buf)){
			...
			throw new Picasa_Exception_UnauthorizedException($message, $buf);
			...
			throw new Picasa_Exception_BadRequestException($message, $buf);
		} else if (preg_match("/500 INTERNAL/i", $buf)){
			if ($message == null) {
			    	$message = "An error occured on the Picasa servers.";
			}
			throw new Picasa_Exception_InternalServerErrorException($message, $buf);
		} else if (preg_match("/409 CONFLICT/i", $buf)){
			if ($message == null) {
			    	$message = "An error occured on the Picasa servers.";
			}
			throw new Picasa_Exception_ConflictException($message, $buf);
		} else {
			if ($message == null) {
			    	$message = "An unknown error was encountered.";
			}
	    		throw new Picasa_Exception($message, $buf);
		}
	}
