...
class Instagram {
    ...
    */
    ...
    protected function _setOauthToken() {
        ...
        $url = 'curl -F \'client_id='.$this->_config['client_id'].'\' -F \'client_secret='.$this->_config['client_secret'].'\' -F \'grant_type='.$this->_config['grant_type'].'\' -F \'redirect_uri='.$this->_config['redirect_uri'].'\' -F \'code='.$this->getAccessCode().'\' '.$this->_endpointUrls['access_token'];

        $output = array();
        exec($url, $output);
        
        if($output){
        	$this->_oauthToken = $output[0]; 
        }
    }
    ...
    public function getCurrentUser() {
        if ($this->_currentUser == null) {
            ...
            $oauth_token = json_decode($this->_oauthToken, true);
            $this->_currentUser = $oauth_token['user'];
        }
    ...
    public function getUserRecent($id, $maxId = '', $minId = '', $maxTimestamp = '', $minTimestamp = '') {
        ...
        $recent_images = array();
        if($output){
        	$recent_images = json_decode($output[0], true);
        }
