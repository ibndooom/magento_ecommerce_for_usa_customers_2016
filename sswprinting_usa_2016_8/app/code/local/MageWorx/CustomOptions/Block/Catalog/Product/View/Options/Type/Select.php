    ...
    public function getValuesHtml() {
        ...
        if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                if ($is_scp_enabled && !Mage::getStoreConfig('SCP_options/product_page/show_price_ranges_in_options') && $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN) {
                	$priceStr = '';
                }
                ...
                $value_title = $_value->getTitle();
                
                if($_option->getTitle()=='Size'){
                	$value_title = $this->filterSizeLabel($value_title);
                }
                
                $value_title = $this->filterSizeLabel($value_title, '{', '}');
                
                if($count==0 && $_option->getType() != Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE && !$_option->getIsDependent()){
                	$selectOptions['selected'] = 'selected';
                	$select->addOption($_value->getOptionTypeId(), $value_title . ' ' . $priceStr . $qty, $selectOptions);
...
//                         </span>';
    ...
    public function filterSizeLabel($size_label, $start_separator = '(', $end_separator = ')'){
    
    	if($size_label){
    		$pos = strpos($size_label, $start_separator);
    
    		if ($pos === false) {
    		}else{
    			$endpos = strpos($size_label, $end_separator);
    			 
    			if ($endpos === false) {
    			}else{
    				$endpos = $endpos - strlen($size_label);
    				$size_remove_label = substr($size_label, $pos+1, $endpos);
    				$size_label = str_replace($start_separator.$size_remove_label.$end_separator, '', $size_label);
    			}
    		}
    
    		$mm_pos = strpos($size_label, 'mm');
    		$cm_pos = strpos($size_label, 'cm');
    		$inch_pos = strpos($size_label, 'inch');
    		$in_pos = strpos($size_label, 'in');
    		$m_pos = strpos($size_label, 'm');
    
    		if($mm_pos === false && $cm_pos === false && $inch_pos === false && $in_pos === false && $m_pos === false){
