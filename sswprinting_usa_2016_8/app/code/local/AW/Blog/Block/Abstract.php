...
abstract class AW_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    const DEFAULT_READ_MORE_COUNT = '300';
            ...
            if ($shortContent) {
                if ($item->getShortContent()) {
                    ...
                    $strManager = new AW_Blog_Helper_Substring(
                        array(
                            'input' => self::$_helper->filterWYS($content)
                        )
                    );
                    if($readMoreCount){
                        $substrContent = $strManager->getHtmlSubstr($readMoreCount);

                    }else{
                        $substrContent = $strManager->getHtmlSubstr(self::DEFAULT_READ_MORE_COUNT);

                    }
                    $item->setPostContent($substrContent);
                }
    ...
    {
        switch($format){
            case Mage_Core_Model_Locale::FORMAT_TYPE_SHORT :
                $_date = Mage::getModel('core/date')->date('d/m/Y G:i:s', strtotime($date));
                break;
            case Mage_Core_Model_Locale::FORMAT_TYPE_MEDIUM :
                $_date = Mage::getModel('core/date')->date('M d, Y G:i:s', strtotime($date));
                break;
            case Mage_Core_Model_Locale::FORMAT_TYPE_LONG :
                $_date = Mage::getModel('core/date')->date('F d, Y G:i:s', strtotime($date));
                break;
            case Mage_Core_Model_Locale::FORMAT_TYPE_FULL :
                $_date = Mage::getModel('core/date')->date('l, F j,Y G:i:s', strtotime($date));
                break;
        }
